...
def runANN():
    ...
    text.insert(END, "Total Features : " + str(total) + "\n")
    text.insert(END, "Features set reduce after applying features selection concept : " + str((total - X_train_selected.shape[1])) + "\n\n")
    ...
    model.fit(X_train_selected, y_train, epochs=100, batch_size=32)
    
    _, ann_acc = model.evaluate(X_train_selected, y_train)
    ann_acc = ann_acc * 100
